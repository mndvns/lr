...
LR.prototype.watch = function(path, command, ignore) {
  this._watchers[path] = {cmd: command, ignore: ignore};
  this.emit('watch', path, command, ignore);
  return this;
};
...
LR.prototype.start = function(fn) {
  ...
  this._watcher = watch(this._watchers, opts, function(path, ignore) {
    self.emit('change', path, ignore);
